...
import static utils.HelpMethods.isProjectileHitingLevel;
...
public class ObjectManager {
	...
	private void loadImgs() {
		BufferedImage potionSprite = LoadSave.GetSpriteAtlas(LoadSave.POITION_ATLAS);
		...
		for(Projectile p:projectiles)
			if(p.isActive()) {
				...
				if(p.getHitbox().intersects(player.getHitbox())) {
					...
					p.setActive(false);
				}else if(isProjectileHitingLevel(p,lvlData))
					p.setActive(false);
			}
	}

	private boolean isPlayerInRange(Cannon c, Player player) {
		int absValue =(int)Math.abs(player.getHitbox().x-c.x);
		...
		for(Projectile p:projectiles)
			if(p.isActive())
				g.drawImage(cannonBallImg, (int)(p.getHitbox().x-xLvlOffset),
						(int)(p.getHitbox().y), CANNON_BALL_WIDTH, 
